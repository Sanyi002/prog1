...
private:
    ...
    LZWBinFa ( const LZWBinFa & regi ) {
        std::cout << "LZWBinFa copy" << std::endl;
...
main (int argc, char *argv[])
{
    ...
    std::fstream beFile (barmi, std::ios_base::in);

    // fejlesztgetjük a forrást: httpkiFile://progpater.blog.hu/2011/04/17/a_tizedik_tizenegyedik_labor
    if (!beFile)
    {
    ...
    std::string ki = argv[3];

    std::fstream kiFile (ki, std::ios_base::out);
